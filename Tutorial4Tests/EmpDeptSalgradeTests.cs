...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => depts.Where(dept => dept.Loc == "CHICAGO")
            .Select(dept => dept.DeptNo).Contains(e.DeptNo)).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.Join(grades, e => true, g => true, (emp, grd) => new {emp, grd})
            .Where(eg => eg.emp.Sal >= eg.grd.Losal && eg.emp.Sal <= eg.grd.Hisal).Select(eg => new
            {
                eg.emp.EName,
                eg.grd.Grade
            });
